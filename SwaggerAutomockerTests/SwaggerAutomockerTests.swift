//
//  SwaggerAutomockerTests.swift
//  SwaggerAutomockerTests
//
...
import XCTest
@testable import SwaggerAutomocker

class SwaggerAutomockerTests: XCTestCase {
...
extension BaseTest {
    ...
    fileprivate func post(path: String, body: Any = [:]) -> URLRequest {
        ...
        request.setValue("Application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = try? JSONSerialization.data(withJSONObject: body, options: [.prettyPrinted, .sortedKeys, .fragmentsAllowed])
        return request
    }
    ...
    fileprivate func serverURL(path: String = "") -> URL {
        ...
        components.path = (mockServer.swagger?.basePath ?? "") + path
        return components.url!
    }
    
    fileprivate func formatDictionary(_ dictionary: [String: Any]) -> [String: Any] {
        if let data = try? JSONSerialization.data(withJSONObject: dictionary, options: [.prettyPrinted, .sortedKeys, .fragmentsAllowed]) {
